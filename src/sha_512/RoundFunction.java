...
public class RoundFunction {
	...
	public long summation0To512(long x) {
		
		return ROTR(x,28)^ROTR(x,34)^ROTR(x,39);
	}
	
	public long summation1To512(long x) {
		
		return ROTR(x,14)^ROTR(x,18)^ROTR(x,41);
	}
	
	public long sigma0To512(long x) {
		
		return ROTR(x,1)^ROTR(x,8)^(x >>> 7);
	}
	
	public long sigma1To512(long x) {
	...
	public String hash(byte [] input) {
		...
		MessagePadding msgPad = new MessagePadding();
		
		input = msgPad.padding(input);
		
		long [][]block = msgPad.block_function(input);
		
		long [][]w = msgPad.processMessage(block);
		...
		for(int i=0 ; i<block.length ; i++) {
			...
			for(int j=0 ; j<80 ; j++) {
				...
				long t1=h+Ch(e, f, g)+summation1To512(e)+w[i][j]+k[j];
				long t2=summation0To512(a)+Maj(a, b, c);
			...
			hashBuffer[0] = a + hashBuffer[0];
			hashBuffer[1] = b + hashBuffer[1];
			hashBuffer[2] = c + hashBuffer[2];
			hashBuffer[3] = d + hashBuffer[3];
			hashBuffer[4] = e + hashBuffer[4];
			hashBuffer[5] = f + hashBuffer[5];
			hashBuffer[6] = g + hashBuffer[6];
			hashBuffer[7] = h + hashBuffer[7];
			
		}
		
		for (int i = 0; i < 8; i++) {
			
			s+=String.format("%016x", hashBuffer[i]);
		}
